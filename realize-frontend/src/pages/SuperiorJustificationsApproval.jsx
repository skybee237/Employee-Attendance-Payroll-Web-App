...
const SuperiorJustificationsApproval = ({ superiorId }) => {
  ...
  const fetchJustifications = async () => {
    try {
      setLoading(true);
      const response = await API.get(`/api/superior/justifications/${superiorId}`);
      setJustifications(response.data);
    } catch (err) {
      console.error("Failed to fetch subordinate justifications:", err);
  ...
  const handleStatusUpdate = async (justificationId, newStatus) => {
    try {
      const response = await API.put(`/api/superior/approve-justification/${superiorId}/${justificationId}`, {
        status: newStatus
      });
      toast.success(response.data.message);
      fetchJustifications(); // Refresh the list
    } catch (err) {
      console.error("Failed to update justification status:", err);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">Justifications Approval</h1>
        <button
            ...
            filteredJustifications.map((justification) => (
              <div key={justification._id} className="border rounded-lg p-6 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-500">
                      <span>📝 Type: {justification.type || 'Regular'}</span>
                      <span>📅 Submitted: {formatDate(justification.createdAt)}</span>
                    </div>
                  </div>
                ...
                {justification.comments && (
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-700">
                      <strong>Comments:</strong> {justification.comments}
                    </p>
                  </div>
                )}
              </div>
...
export default SuperiorJustificationsApproval;
